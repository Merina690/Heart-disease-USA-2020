...
# checking the imbalance data in target variable

# In[27]:


heart_df['HeartDisease'].value_counts() #printing values in heart disease
...
y=heart_df.HeartDisease #separation of dependent and independent features
...
#train test split = splitting the training and testing data randomly from the extracted data
...
y_train.value_counts()#checking value counts for y_train


# ### BALANCING THE DATA USING OVER SAMPLING

# We have various techniques to balance the data like undersampling, over sampling, combination of both

# In[31]:


from imblearn.over_sampling import SMOTE #importing smote library
sm = SMOTE(random_state=42) #assigning a variable 
x, y = sm.fit_resample(x, y) #fitting and resampling the data


# In[32]:


x_train,x_test,y_train,y_test=train_test_split(x,y,test_size=0.3,random_state=40) # train test split


# In[33]:


y_train.value_counts() # the data is balanced using smote over sampling
